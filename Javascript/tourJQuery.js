...
function showMarker(results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
        ...
        latTextBox = $("#placeDetailsView tr:eq(4) td:eq(1)").find("input");
        lngTextBox = $("#placeDetailsView tr:eq(5) td:eq(1)").find("input");

        $("#placeDetailsView tr:eq(0) td:eq(1)").find("input").val(address);
